...
import cmput301.f13t01.readstory.ReadFragmentActivity;

/**
 * Sets up and handles browse online ui that allows user to search for and
 * display stories online, select an online story to read from beginning, select
 * an online story to save, or select aznd read a random online story
 * 
...
public class BrowseOnlineStoriesActivity extends Activity {
	 ...
	 *            The state of activity last kill
	 */
	@Override
	public void onCreate(Bundle savedInstanceState) {
		...
		objLibrary = GlobalManager.getLocalManager();
		esLibrary = GlobalManager.getESManager();
	...
	/**
	 * Resume activity clears the search input boxes and display any online
	 * stories
	 */
	public void onResume() {
		...
		clearInputBoxes();
		// get all the story infos list from es manager for the story list
		// adapter
		results = esLibrary.getStoryInfoList(0);
		// initialize adapter and update the view
		objStoryAdapter = new StoryInfoListAdapter(this,
				R.layout.story_info_list_item, results);
	 ...
	 * 
	 * @param menu
	 *            The action bar menu xml resource to inflate into action bar
	 ...
	 * 
	 * @param item
	 *            The menu item resource selected by user
	 * @return boolean Returns true for successful handling of menu item
	 *         selection, false otherwise (from superclass)
	 */
	@Override
	 ...
	 * 
	 * @param menu
	 *            The menu xml resource to inflate into contextual menu
	 * @param v
	 *            The view in which create the contextual menu
	 * @param menuInfo
	 *            The additional info about item selected
	 */
	@Override
	...
	private void startSearchOnline() {
		// feedback to user, echoing menu text
		Toast toast = Toast.makeText(getApplicationContext(), getResources()
				.getString(R.string.action_search_online_stories),
				Toast.LENGTH_SHORT);
		toast.show();
	...
	private void onSelectHelp() {
		...
		android.app.DialogFragment newFragment = (android.app.DialogFragment) HelpFragment
				.newInstance(HelpMessage.BROWSE_ONLINE);
		newFragment.show(ft, "help_dialog");
	}

	/**
	 * Starts reading story at beginning child activity storyId is UUID of story
	 * selected by user
	 */
	...
	private void startRandomStory() {
		...
		// then we open the locally save story for local reading with the new
		// UUID
