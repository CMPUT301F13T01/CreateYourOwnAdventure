...
import cmput301.f13t01.editstory.EditStoryActivity;
...
/**
 * Sets up and handles main screen ui that allows user to display stories saved
 * on device, select a story to read from beginning, select a story to read from
 * last history, select a story to edit a story, select a story to delete,
...
public class MainActivity extends Activity {
	 ...
	 *            The state of activity at last kill time
	 */
	@Override
	...
	protected void onResume() {
		...
		objStoryAdapter = new StoryInfoListAdapter(this,
				R.layout.story_info_list_item, storyInfoList);
	...
	private void onSelectHelp() {
		...
		android.app.DialogFragment newFragment = (android.app.DialogFragment) HelpFragment
				.newInstance(HelpMessage.MAIN_SCREEN);
	...
	private void startRandomStory() {
		...
		if (randStorySource.size() <= 0) {
			...
			intent.putExtra(getResources().getString(R.string.story_continue),
					true);
			intent.putExtra(getResources().getString(R.string.story_id),
					randStoryId);
